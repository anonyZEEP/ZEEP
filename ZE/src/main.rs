...
fn uptil_DGSA(
    ...
    for i in 0..k {
        ...
        let start_en_e = Instant::now();

        let sig_e_v1 = ea.SIG_sig(veh.v_id, &veh.v_pk);

        let duration_en_e = start_en_e.elapsed();

        println!("Time taken for enrollment (EA) : {:.2?}", duration_en_e);
        ...
        println!("Time taken for enrollment (Vehicle) : {:.2?}", duration_en);

        let start_auth_ia = Instant::now();
        ...
        let duration_auth_ia = start_auth_ia.elapsed();

        println!("Time taken for authorization (IA) : {:.2?}", duration_auth_ia);

        let start_auth = Instant::now();

        let cred_v1 = veh.get_cred(&sigma_v1.unwrap(), &ia.IAPublicKey, epoch);

        let duration_auth = start_auth.elapsed();

        println!("Time taken for authorization (vehicle) : {:.2?}", duration_auth);
...
fn after_DGSA(
    ...
    println!("Time taken for enter: {:.2?}", duration.as_millis());
